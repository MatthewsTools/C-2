...
*/
int csq::x86_gen::data_type_size(token tok)
{
  switch(tok.token){
    case KEY_VOID :
    case KEY_CHAR :
...
*/
int csq::x86_gen::data_decl_size(declspace_t ds)
{
  switch(ds){
    case DB : return 1;
    case DW : return 2;
...
*/
int csq::x86_gen::resv_decl_size(resspace_t rs)
{
  switch(rs){
    case RESB : return 1;
    case RESW : return 2;
...
*/
declspace_t csq::x86_gen::declspace_type_size(token tok)
{
...
*/
resspace_t csq::x86_gen::resvspace_type_size(token tok)
{
  int sz = data_type_size(tok);
  switch(sz){
    case 1:
...
*/
bool csq::x86_gen::has_float(struct primary_expr* pexpr)
{
  token type;
...
*/
void csq::x86_gen::max_datatype_size(struct primary_expr* pexpr, int *dsize)
{
  token type;
  int dsize2 = 0;
...
*/
void csq::x86_gen::get_func_local_members()
{
...
st_symbol_info* csq::x86_gen::search_func_params(std::string str)
{
...
insnsize_t csq::x86_gen::get_insn_size_type(int sz)
{
  if(sz == 1)
    return BYTE;
  else if(sz == 2)
...
csq::x86_gen::get_post_order_prim_expr(struct primary_expr *pexpr)
{
...
struct insn* csq::x86_gen::get_insn(insn_t instype, int oprcount)
{
...
void csq::x86_gen::insert_comment(std::string cmnt)
{
...
struct data* csq::x86_gen::search_data(std::string dt)
{
...
//search given data in data section vector
struct data* csq::x86_gen::search_string_data(std::string dt)
{
...
std::string csq::x86_gen::hex_escape_sequence(char ch)
{
  switch(ch){
    case '\'': return "0x27";
    case '"': return "0x22";
...
std::string csq::x86_gen::get_hex_string(std::string str)
{
...
bool csq::x86_gen::get_function_local_member(struct func_member* fmemb, token tok)
{
...
insn_t csq::x86_gen::get_arthm_op(lexeme_t symbol)
{
  if(symbol == "+")
    return ADD;
  else if(symbol == "-")
...
e.g: x = x;
*/
regs_t csq::x86_gen::gen_int_primexp_single_assgn(struct primary_expr* pexpr, int dtsize)
{
...
*/
bool csq::x86_gen::gen_int_primexp_compl(struct primary_expr* pexpr, int dtsize)
{
  struct insn* in = nullptr;
  struct func_member fmem;
  if(pexpr == nullptr) return false;
...
struct data* csq::x86_gen::create_string_data(std::string value)
{
...
*/
regs_t csq::x86_gen::gen_string_literal_primary_expr(struct primary_expr *pexpr)
{
  if(pexpr == nullptr) return RNONE;
  if(pexpr->left == nullptr && pexpr->right == nullptr){
...
*/
regs_t csq::x86_gen::gen_int_primary_expression(struct primary_expr *pexpr)
{
  std::stack<struct primary_expr*> pexp_stack;
  std::stack<struct primary_expr*> pexp_out_stack;
...
insn_t csq::x86_gen::get_farthm_op(lexeme_t symbol, bool reverse_ins)
{
  if(symbol == "+"){
    return FADD;
  }else if(symbol == "-"){
...
struct data* csq::x86_gen::create_float_data(declspace_t ds, std::string value)
{
...
fregs_t csq::x86_gen::gen_float_primexp_single_assgn(struct primary_expr* pexpr, declspace_t decsp)
{
  struct insn* in = nullptr;
  fregs_t rs = FRNONE;
...
*/
void csq::x86_gen::gen_float_primary_expression(struct primary_expr *pexpr)
{
  std::stack<struct primary_expr*> pexp_stack;
  std::stack<struct primary_expr*> pexp_out_stack;
  primary_expr* pexp = nullptr, *fact1 = nullptr, *fact2 = nullptr;
...
*/
std::pair<int, int>
csq::x86_gen::gen_primary_expression(struct primary_expr ** pexpr)
{
  regs_t result;
...
//e.g: x = 1 + 2 *3 ;
void csq::x86_gen::gen_assgn_primary_expr(struct assgn_expr** asexpr)
{
...
*/
void csq::x86_gen::gen_sizeof_expression(struct sizeof_expr** sofexpr)
{
...
void csq::x86_gen::gen_assgn_sizeof_expr(struct assgn_expr** asexpr)
{
  struct assgn_expr* assgnexp = *asexpr;
  struct insn* in = nullptr;
  int dtsize = 0;
...
void csq::x86_gen::gen_assgn_cast_expr(struct assgn_expr** asexpr)
{
  struct assgn_expr* assgnexp = *asexpr;
  struct insn* in = nullptr;
  int dtsize = 0;
...
*/
void csq::x86_gen::gen_id_expression(struct id_expr** idexpr)
{
...
void csq::x86_gen::gen_assgn_id_expr(struct assgn_expr** asexpr)
{
  struct assgn_expr* assgnexp = *asexpr;
  struct insn* in = nullptr;
  int dtsize = 0;
...
void csq::x86_gen::gen_assgn_funccall_expr(struct assgn_expr** asexpr)
{
  struct assgn_expr* assgnexp = *asexpr;
  struct insn* in = nullptr;
  int dtsize = 0;
...
void csq::x86_gen::gen_assignment_expression(struct assgn_expr** asexpr)
{
  struct assgn_expr* assgnexp = *asexpr;
...
*/
void csq::x86_gen::gen_funccall_expression(struct func_call_expr** fccallex)
{
  struct insn* in = nullptr;
  int pushed_count = 0;
  int param_count  = 0;
...
void csq::x86_gen::gen_cast_expression(struct cast_expr** cexpr)
{
...
void csq::x86_gen::gen_expression(struct expr** __expr)
{
...
void csq::x86_gen::gen_label_statement(struct labled_stmt** labstmt)
{
...
void csq::x86_gen::gen_jump_statement(struct jump_stmt** jstmt)
{
...
regs_t csq::x86_gen::get_reg_type_by_char(char ch)
{
  switch(ch){
    case 'a': return EAX;
    case 'b': return EBX;
...
std::string csq::x86_gen::get_asm_output_operand(struct asm_operand** asmoprnd)
{
...
std::string csq::x86_gen::get_asm_input_operand(struct asm_operand** asmoprnd)
{
  struct asm_operand* asmoperand = *asmoprnd;
...
void csq::x86_gen::get_nonescaped_string(std::string& str)
{
  size_t fnd;
  fnd = str.find("\\t");
...
void csq::x86_gen::gen_asm_statement(struct asm_stmt** _asmstm)
{
...
bool csq::x86_gen::is_literal(token tok)
{
  token_t t = tok.token;
...
bool csq::x86_gen::gen_float_type_condition(struct primary_expr** f1, struct primary_expr **f2,
...
token_t csq::x86_gen::gen_select_stmt_condition(struct expr* _expr)
{
  ...
  switch(_expr->expr_kind){
    ...
    default: csq::error::print_error(csq::filename,
...
void csq::x86_gen::gen_selection_statement(struct select_stmt** slstmt)
{
...
void csq::x86_gen::gen_iteration_statement(struct iter_stmt** istmt)
{
...
void csq::x86_gen::gen_statement(struct stmt** _stmt)
{
...
void csq::x86_gen::save_frame_pointer()
{
...
*/
void csq::x86_gen::restore_frame_pointer()
{
  insn* in = get_insn(INSLABEL, 0);
...
*/
void csq::x86_gen::gen_uninitialized_data()
{
  int i;
  struct st_symbol_info* temp = nullptr;
  std::list<token>::iterator it;

  if(csq::global_symtab == nullptr) return;

  for(i = 0; i < ST_SIZE; i++){
    temp = csq::global_symtab->symbol_info[i];
        ...
        if(temp->is_array){
          if(temp->arr_dimension_list.size() > 1){
            ...
            while(it != temp->arr_dimension_list.end()){
              rv->res_size *= csq::get_decimal(*it);
              it++;
            }
          }else{
            rv->res_size = csq::get_decimal(*(temp->arr_dimension_list.begin()));
          }
        }else{
          if(rv->res_size < 1)
            rv->res_size = 1;
...
void csq::x86_gen::gen_array_init_declaration(struct st_node* symtab)
{
  int i;
  struct st_symbol_info* syminf = nullptr;
  struct data* dt = nullptr;
        ...
        for(auto e1 : syminf->arr_init_list){
          for(auto e2 : e1){
            if(e2.token == LIT_FLOAT){
              ...
              dt->array_data.push_back(std::to_string(csq::get_decimal(e2)));
            }
          }
        }
        data_section.push_back(dt);
...
void csq::x86_gen::gen_record()
{
  ...
  if(csq::record_table == nullptr) return;
  for(int i = 0; i < ST_RECORD_SIZE; i++){
    recnode = csq::record_table->recordinfo[i];
...
*/
void csq::x86_gen::gen_global_declarations(struct tree_node** trnode)
{
  ...
  gen_array_init_declaration(csq::global_symtab);
...
void csq::x86_gen::write_text_to_asm_file(std::ofstream& outfile)
{
...
void csq::x86_gen::write_record_member_to_asm_file(struct record_data_type& x,
...
void csq::x86_gen::write_record_data_to_asm_file(struct resv** rv,
                                                  std::ofstream& outfile)
{
  struct resv* r = *rv;
  if(r == nullptr) return;
...
void csq::x86_gen::write_data_to_asm_file(std::ofstream& outfile)
{
  if(!outfile.is_open()) return;
...
void csq::x86_gen::write_resv_to_asm_file(std::ofstream& outfile)
{
  if(!outfile.is_open()) return;
...
void csq::x86_gen::write_instructions_to_asm_file(std::ofstream& outfile)
{
  std::string cast;
  if(!outfile.is_open()) return;
...
void csq::x86_gen::write_asm_file()
{
...
bool csq::x86_gen::search_text(struct text* tx)
{
...
void csq::x86_gen::gen_x86_code(struct csq::tree_node** ast)
{
  struct tree_node *trhead = *ast;
  if(trhead == nullptr) return;

  if(optimize){
    optmz = new csq::optimizer;
    optmz->optimize(&trhead);
    delete optmz;
    optmz = nullptr;
    if(csq::error_count > 0) return;
  }
