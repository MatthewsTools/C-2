...
struct csq::tree_node* ast = nullptr;
...
{
  if(csq::error_count > 0){
    csq::tree::delete_tree(&ast);
    csq::symtable::delete_node(&csq::global_symtab);
    csq::symtable::delete_record_symtab(&csq::record_table);
    return false;
  }
  return true;
}
...
bool compile(std::string filename)
{
  ...
  csq::lex = new csq::lexer(filename);

  //create parser object
  csq::parser *p = new csq::parser();
  ...
  csq::analyzer *an = new csq::analyzer();
  ...
  csq::x86_gen *x86 = new csq::x86_gen;
  ...
  if(csq::error_count == 0){
    if(print_tree){
      ...
      csq::print::print_tree(ast, false);
    }
    if(print_symtab){
      std::cout<<"file: "<<filename<<std::endl;
      csq::print::print_symtab(csq::global_symtab);
    }
    if(print_record_symtab){
      std::cout<<"file: "<<filename<<std::endl;
      csq::print::print_record_symtab(csq::record_table);
    }
  }
