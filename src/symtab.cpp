...
struct st_type_info* csq::symtable::get_type_info_mem()
{
  struct st_type_info* newst = new struct st_type_info;
  return newst;
}

struct st_rec_type_info* csq::symtable::get_rec_type_info_mem()
{
  struct st_rec_type_info* newst = new struct st_rec_type_info;
  return newst;
}

struct st_symbol_info* csq::symtable::get_symbol_info_mem()
{
...
struct st_func_param_info* csq::symtable::get_func_param_info_mem()
{
...
struct st_func_info* csq::symtable::get_func_info_mem()
{
...
struct st_node* csq::symtable::get_node_mem()
{
  unsigned i;
...
struct st_record_node* csq::symtable::get_record_node_mem()
{
...
struct st_record_symtab* csq::symtable::get_record_symtab_mem()
{
  unsigned i;
...
void csq::symtable::delete_type_info(struct st_type_info **stinf)
{
...
void csq::symtable::delete_rec_type_info(struct st_rec_type_info **stinf)
{
  if(*stinf == nullptr) return;
  //delete *stinf;
  *stinf = nullptr;
...
void csq::symtable::delete_symbol_info(struct st_symbol_info **stinf)
{
  if(*stinf == nullptr) return;
...
void csq::symtable::delete_func_param_info(struct st_func_param_info** stinf)
{
  if(*stinf == nullptr) return;
...
void csq::symtable::delete_func_info(struct st_func_info** stinf)
{
  if(*stinf == nullptr) return;
...
void csq::symtable::delete_node(struct st_node** stinf)
{
...
void csq::symtable::delete_record_node(struct st_record_node** stinf)
{
...
void csq::symtable::delete_record_symtab(struct st_record_symtab** stinf)
{
...
//hashing functions
unsigned int csq::symtable::st_hash_code(lexeme_t lxt)
{
  void *key = (void*)lxt.c_str();
  unsigned int murhash = csq::murmurhash2(key, lxt.size(), 4);
  return murhash%ST_SIZE;
}

unsigned int csq::symtable::st_rec_hash_code(lexeme_t lxt)
{
  void *key = (void*)lxt.c_str();
  unsigned int murhash = csq::murmurhash2(key, lxt.size(), 4);
...
//table operations
void csq::symtable::add_sym_node(struct st_symbol_info** symnode)
{
  struct st_symbol_info* temp = *symnode;
  if(temp == nullptr){
    *symnode = get_symbol_info_mem();
    csq::last_symbol = *symnode;
  }else{
    while(temp->p_next != nullptr){
      temp = temp->p_next;
    }
    temp->p_next = get_symbol_info_mem();
    csq::last_symbol = temp->p_next;
  }
}

void csq::symtable::insert_symbol(struct st_node** symtab, lexeme_t symbol)
{
...
bool csq::symtable::search_symbol(struct st_node* st, lexeme_t symbol)
{
...
struct st_symbol_info* csq::symtable::search_symbol_node(struct st_node* st, lexeme_t symbol)
{
...
void csq::symtable::insert_symbol_node(struct st_node** symtab, struct st_symbol_info** syminf)
{
  struct st_symbol_info* temp = nullptr;
  if(*symtab == nullptr) return;
  if(*syminf == nullptr) return;

  temp = csq::symtable::search_symbol_node(*symtab, (*syminf)->symbol);
  if(temp != nullptr)
    temp = *syminf;
}

bool csq::symtable::remove_symbol(struct st_node** symtab, lexeme_t symbol)
{
  struct st_symbol_info* temp = nullptr;
...
void csq::symtable::add_rec_node(struct st_record_node** recnode)
{
  struct st_record_node* temp = *recnode;
  if(temp == nullptr){
    *recnode = get_record_node_mem();
    csq::last_rec_node = *recnode;
  }else{
    while(temp->p_next != nullptr)
      temp = temp->p_next;

    temp->p_next = get_record_node_mem();
    csq::last_rec_node = temp;
  }
}

void csq::symtable::insert_record(struct st_record_symtab** recsymtab,
                                    record_t recordname)
{
...
bool csq::symtable::search_record(struct st_record_symtab* rec, record_t recordname)
{
...
struct st_record_node* csq::symtable::search_record_node(struct st_record_symtab* rec,
                                                          record_t recordname)
{
