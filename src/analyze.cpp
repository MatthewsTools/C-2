...
st_symbol_info* csq::analyzer::search_func_params(token tok)
{
...
*/
void csq::analyzer::check_invalid_type_declaration(struct st_node *symtab)
{
  unsigned i;
...
*/
bool csq::analyzer::check_pointer_arithmetic(
                                  ...
                                  struct primary_expr* fact_2)
{
  ...
  if(fact_1->is_id && !fact_2->is_id
    ...
    if(opr->tok.token == ARTHM_ADD || opr->tok.token == ARTHM_SUB){
      ...
      csq::error::print_error(csq::filename, "invalid operand to binary "
...
*/
bool csq::analyzer::check_primexp_type_argument(
                          struct primary_expr* opr,
                          struct primary_expr* fact_1,
                          struct primary_expr* fact_2)
{
...
*/
bool csq::analyzer::check_unary_primexp_type_argument(struct primary_expr* pexpr)
{
  bool result = true;
  st_symbol_info* syminf = nullptr;
  ...
  if(pexpr->is_id){
    ...
    if(syminf == nullptr){
      csq::error::print_error(csq::filename, "undeclared '"
...
bool csq::analyzer::check_unary_idexp_type_argument(struct id_expr* idexpr)
{
  bool result = true;
  st_symbol_info* syminf = nullptr;
...
if not, then error
*/
bool csq::analyzer::check_array_subscript(struct id_expr* idexpr)
{
  bool result = true;
  st_symbol_info* syminf = nullptr;
...
*/
void csq::analyzer::analyze_primary_expr(struct primary_expr** _pexpr)
{
...
*/
void csq::analyzer::analyze_id_expr(struct id_expr** _idexpr)
{
  ...
  while(i+1 < idexp_vec.size()){
    if(i < idexp_vec.size() &&
      ...
      i+2 < idexp_vec.size()){
        ...
        if(idobj->is_id){
          record = csq::symtable::search_record_node(csq::record_table,
                            recordname);

          if(record != nullptr){
            syminf = csq::symtable::search_symbol_node(record->symtab, idobj->tok.lexeme);
            if(syminf != nullptr){
              idobj->id_info = syminf;
              recordname = idobj->id_info->type_info->type_specifier.record_type.lexeme;
            }
        ...
        if(idobj->id_info != nullptr){
          switch(idobj->id_info->type_info->type){
            ...
            case RECORD_TYPE :
              {
                record = csq::symtable::search_record_node(csq::record_table, recordname);
                if(record != nullptr && idmember != nullptr){
                  if(!csq::symtable::search_symbol(record->symtab,
                      idmember->tok.lexeme)){
...
void csq::analyzer::analyze_sizeof_expr(struct sizeof_expr** _szofexpr)
{
...
void csq::analyzer::analyze_cast_expr(struct cast_expr** _cstexpr)
{
...
void csq::analyzer::get_idexpr_idinfo(struct id_expr* idexpr,
...
returns symbol
*/
struct id_expr* csq::analyzer::get_idexpr_attrbute_node(struct id_expr** _idexpr)
{
  std::stack<struct id_expr*> idexp_stack;
  std::vector<struct id_expr*> idexp_vec;
  id_expr* idexp_root = *_idexpr;
    ...
    if(i < idexp_vec.size() &&
      ...
      i+2 < idexp_vec.size()){
        ...
        if(idobj->is_id){
          ...
          if(record != nullptr){
            syminf = csq::symtable::search_symbol_node(record->symtab,
                                                        idmember->tok.lexeme);
            if(syminf != nullptr){
...
int csq::analyzer::tree_height(int exprtype,
...
*/
struct id_expr* csq::analyzer::get_assgnexpr_idexpr_attribute(struct id_expr* _idexp)
{
...
when %=, &=, |=, ^=, <<=, >>=
*/
bool csq::analyzer::check_assignment_type_argument(struct assgn_expr* assgnexpr,
                                                    int type,
                                                    struct id_expr* idexpr,
                                                    struct primary_expr* pexpr)
{
...
void csq::analyzer::simplify_assgn_primary_expression(struct assgn_expr** asexpr)
{
  ...
  left = csq::tree::get_primary_expr_mem();
  left->is_id = true;
  left->tok = assgnexp->id_expression->tok;
  left->is_oprtr = false;
  left->id_info = search_id(left->tok);

  opr = csq::tree::get_primary_expr_mem();
  opr->is_oprtr = true;
...
*/
void csq::analyzer::analyze_assgn_expr(struct assgn_expr** _assgnexpr)
{
    ...
    case FUNC_CALL_EXPR :
      {
        ...
        findit = csq::func_table.find(assgnexpr->expression->func_call_expression->function->tok.lexeme);
        if(findit == csq::func_table.end()) return;
...
*/
void csq::analyzer::analyze_funccall_expr(struct func_call_expr** _funcallexpr)
{
  ...
  findit = csq::func_table.find(funcexpr->function->tok.lexeme);
  if(findit == csq::func_table.end()){
    csq::error::print_error(csq::filename,
...
void csq::analyzer::analyze_expression(struct expr** __expr)
{
...
*/
void csq::analyzer::analyze_label_statement(struct labled_stmt** labelstmt)
{
...
void csq::analyzer::analyze_selection_statement(struct select_stmt** selstmt)
{
...
void csq::analyzer::analyze_iteration_statement(struct iter_stmt** iterstmt)
{
...
*/
void csq::analyzer::analyze_return_jmpstmt(struct jump_stmt** jmpstmt)
{
...
*/
void csq::analyzer::analyze_jump_statement(struct jump_stmt** jmpstmt)
{
...
void csq::analyzer::analyze_goto_jmpstmt()
{
...
bool csq::analyzer::is_digit(char ch)
{
  return ((ch - '0' >= 0) && (ch - '0' <= 9));
}

std::string csq::analyzer::get_template_token(std::string asmtemplate)
{
  std::string asmtoken;
  unsigned i;
...
std::vector<int> csq::analyzer::get_asm_template_tokens_vector(token tok)
{
  size_t loc;
  std::vector<int> v;
  std::string asmtoken;
...
void csq::analyzer::analyze_asm_template(struct asm_stmt** asmstmt)
{
...
m memory
*/
void csq::analyzer::analyze_asm_output_operand(struct asm_operand **operand)
{
...
i immediate value
*/
void csq::analyzer::analyze_asm_input_operand(struct asm_operand **operand)
{
  if(*operand == nullptr) return;
  token constrainttok = (*operand)->constraint;
  lexeme_t constraint = constrainttok.lexeme;
...
void csq::analyzer::analyze_asm_operand_expression(struct expr** _expr)
{
...
void csq::analyzer::analyze_asm_statement(struct asm_stmt** asmstmt)
{
  struct asm_stmt* asmstmt2 = *asmstmt;
...
void csq::analyzer::analyze_statement(struct stmt** _stmt)
{
...
*/
void csq::analyzer::analyze_func_param_info(struct st_func_info** funcinfo)
{
...
bool csq::analyzer::has_constant_member(struct primary_expr* pexpr)
{
...
bool csq::analyzer::has_constant_array_subscript(struct id_expr* idexpr)
{
  bool b = true;
...
void csq::analyzer::analyze_global_assignment(struct tree_node** trnode)
{
...
void csq::analyzer::analyze_func_params(struct st_func_info* func_params)
{
...
void csq::analyzer::analyze_local_declaration(struct tree_node** trnode)
{
  struct tree_node* trhead = *trnode;
  if(trhead == nullptr) return;
    ...
    if(trhead->symtab != nullptr){
      ...
      for(struct st_func_param_info* param : func_params->param_list){
        if(param != nullptr && param->symbol_info != nullptr){
          if(csq::symtable::search_symbol(func_symtab, param->symbol_info->symbol)){
            csq::error::print_error(csq::filename,
...
void csq::analyzer::analyze(struct tree_node** trnode)
{
  ...
  check_invalid_type_declaration(csq::global_symtab);
