...
int csq::convert_octal_to_decimal(lexeme_t lx)
{
  size_t sz = lx.size();
...
int csq::convert_hex_to_decimal(lexeme_t lx)
{
  size_t sz = lx.size();
...
int csq::convert_bin_to_decimal(lexeme_t lx)
{
  size_t sz = lx.size();
  int power = 0, num = 0, index = 0;
  if(!lx.empty()){
...
int csq::convert_char_to_decimal(lexeme_t lx)
{
  if(!lx.empty())
    return lx.at(0);
  return 0;
...
std::string csq::decimal_to_hex(unsigned int num)
{
  int temp;
  std::string hex;
