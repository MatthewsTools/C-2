...
*/
bool csq::optimizer::evaluate(token& f1, token& f2, token& op,
                                std::string& stresult, bool has_float)
{
  ...
  if(has_float){
    ...
    d1 = static_cast<double>(csq::get_decimal(f1));
    d2 = static_cast<double>(csq::get_decimal(f2));
  }
  switch(op.token){
    case ARTHM_ADD :
      result = d1 + d2;
...
void csq::optimizer::clear_primary_expr_stack()
{
...
*/
bool csq::optimizer::has_float_type(struct primary_expr* pexpr)
{
  token type;
...
bool csq::optimizer::has_id(struct primary_expr* pexpr)
{
  if(pexpr == nullptr) return false;
  if(pexpr->is_id){
    return true;
...
void csq::optimizer::get_inorder_primary_expr(struct primary_expr** pexpr)
{
...
*/
void csq::optimizer::id_constant_folding(struct primary_expr** pexpr)
{
...
*/
void csq::optimizer::constant_folding(struct primary_expr** pexpr)
{
  struct primary_expr* pexp = *pexpr;
      ...
      if(pexp_eval.size() > 1){
        ...
        if(evaluate(fact1, fact2, opr, stresult, has_float)){
          ...
          if(has_float){
            ...
            if(result < 0){
              stresult = csq::decimal_to_hex(result);
  ...
  if(pexp_eval.size() > 0){
    ...
    csq::tree::delete_primary_expr(&(*pexpr));
    *pexpr = nullptr;
...
bool csq::optimizer::equals(std::stack<struct primary_expr*> st1,
...
*/
struct primary_expr*
csq::optimizer::get_cmnexpr1_node(struct primary_expr** root,
...
*/
void csq::optimizer::change_subexpr_pointers(struct primary_expr** root,
            struct primary_expr** cmn1, struct primary_expr** cmn2)
{
  ...
  if((*root)->right != nullptr){
    ...
    if((*root)->right == *cmn1){
      csq::tree::delete_primary_expr(&(*root)->right);
      (*root)->right = node1;
      return;
    }else{
...
*/
void csq::optimizer::common_subexpression_elimination(struct primary_expr** pexpr)
{
  struct primary_expr* pexp = *pexpr;
...
bool csq::optimizer::is_powerof_2(int n, int* iter)
{
...
*/
void csq::optimizer::strength_reduction(struct primary_expr** pexpr)
{
      ...
      if(!left->is_oprtr && !right->is_oprtr){
        if(!right->is_id){
          if(right->tok.token != LIT_FLOAT){
            decm = csq::get_decimal(right->tok);
...
void csq::optimizer::optimize_primary_expression(struct primary_expr** pexpr)
{
  struct primary_expr* pexp = *pexpr;
  if(pexp == nullptr) return;
...
void csq::optimizer::optimize_assignment_expression(struct assgn_expr** assexpr)
{
...
void csq::optimizer::optimize_expression(struct expr** exp)
{
...
void csq::optimizer::optimize_statement(struct stmt** stm)
{
...
*/
void csq::optimizer::update_count(std::string symbol)
{
...
void csq::optimizer::search_id_in_primary_expr(struct primary_expr* pexpr)
{
  if(pexpr == nullptr) return;
...
void csq::optimizer::search_id_in_id_expr(struct id_expr* idexpr)
{
...
void csq::optimizer::search_id_in_expression(struct expr** exp)
{
  struct expr* exp2 = *exp;
  if(exp2 == nullptr) return;
  switch(exp2->expr_kind){
...
void csq::optimizer::search_id_in_statement(struct stmt** stm)
{
  struct stmt* stm2 = *stm;
  if(stm2 == nullptr) return;
...
*/
void csq::optimizer::dead_code_elimination(struct tree_node** tr)
{
  ...
  for(int i = 0; i < ST_SIZE; i++){
    syminfo = csq::global_symtab->symbol_info[i];
    if(syminfo != nullptr){
  ...
  while(trhead != nullptr){
    if(trhead->symtab != nullptr){
      ...
      while(it != local_members.end()){
        if(it->second == 0){
          csq::symtable::remove_symbol(&func_symtab, it->first);
        }
        it++;
      }
      local_members.clear();
  ...
  while(it != global_members.end()){
    if(it->second == 0){
      csq::symtable::remove_symbol(&csq::global_symtab, it->first);
    }
    it++;
  }
}

void csq::optimizer::optimize(struct tree_node** tr)
{
  struct tree_node* trhead = *tr;
  if(trhead == nullptr) return;
