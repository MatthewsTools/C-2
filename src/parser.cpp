...
*/
/*
* Contains Recursive Descent Parser for csq
...
namespace csq
{
  ...
  struct csq::st_node *global_symtab;
  //global record table
  struct csq::st_record_symtab *record_table;
...
csq::parser::parser()
{
  ...
  csq::global_symtab = csq::symtable::get_node_mem();
  csq::record_table = csq::symtable::get_record_symtab_mem();
...
bool csq::parser::peek_token(token_t tk)
{
...
bool csq::parser::peek_token(std::vector<token_t>& tkv)
{
  token tok = lex->get_next_token();
...
bool csq::parser::peek_token(const char *format...)
{
  va_list args;
...
bool csq::parser::peek_nth_token(token_t tk, int n)
{
...
token_t csq::parser::get_peek_token()
{
  token tok = lex->get_next_token();
...
token_t csq::parser::get_nth_token(int n)
{
  token *tok = new token[n];
  token_t tk;
  int i;
...
//expression literal
bool csq::parser::expr_literal(token_t tkt)
{
...
bool csq::parser::peek_expr_literal_token()
{
  token tok = lex->get_next_token();
...
bool csq::parser::expect_token(token_t tk)
{
  token tok = lex->get_next_token();
  ...
  if(tok.token != tk){
    ...
    if(find_it != token_lexeme_table.end()){
      ...
      csq::error::print_error(csq::filename, "expected ", find_it->second, loc);
...
bool csq::parser::expect_token(token_t tk, bool consume_token)
{
  token tok = lex->get_next_token();
  ...
  if(tok.token != tk){
    ...
    if(find_it != token_lexeme_table.end()){
      ...
      csq::error::print_error(csq::filename, "expected ", find_it->second,
...
bool csq::parser::expect_token(token_t tk, bool consume_token, std::string str)
{
  token tok = lex->get_next_token();

  if(tok.token != tk){
    csq::error::print_error(csq::filename, "expected ", str, tok.loc);
    std::cout<<expr_list<<std::endl;
                                ...
                                std::string str, std::string arg)
{
  ...
  if(tok.token != tk){
    csq::error::print_error(csq::filename, "expected ", str, arg, tok.loc);
    std::cout<<expr_list<<std::endl;
...
bool csq::parser::expect_token(const char *format...)
{
  va_list args;
  va_start(args, format);
  token tok = lex->get_next_token();
  ...
  error::print_error(csq::filename, "expected ", tok.loc);

  return false;
}

void csq::parser::consume_next_token()
{
  lex->get_next_token();
}

void csq::parser::consume_n_tokens(int n)
{
  while(n > 0){
    lex->get_next_token();
    n--;
...
void csq::parser::consume_tokens_till(terminator_t& terminator)
{
  token tok;
...
bool csq::parser::match_with_terminator(terminator_t& tkv, token_t tk)
{
  for(auto x : tkv){
    if(x == tk)
      return true;
...
std::string csq::parser::get_terminator_string(terminator_t& terminator)
{
  std::string st;
  size_t i;
...
unary-operator : one of
  + - ! ~
*/
bool csq::parser::unary_operator(token_t tk)
{
...
bool csq::parser::peek_unary_operator()
{
  token tok = lex->get_next_token();
  token_t tk = tok.token;
  lex->unget_token(tok);
...
binary-operator :
  ...
  bitwise-operator
*/
bool csq::parser::binary_operator(token_t tk)
{
...
arithmetic-operator : one of
  + - * / %
*/
bool csq::parser::arithmetic_operator(token_t tk)
{
  return (tk == ARTHM_ADD || tk == ARTHM_SUB
...
logical-operator : one of
  && ||
*/
bool csq::parser::logical_operator(token_t tk)
{
...
comparison-operator : one of
  < <= > >= == !=
*/
bool csq::parser::comparison_operator(token_t tk)
{
...
*/
bool csq::parser::bitwise_operator(token_t tk)
{
...
assignment-operator : one of
  = += -= *= /= %= |= &= ^= <<= >>=
*/
bool csq::parser::assignment_operator(token_t tk)
{
...
bool csq::parser::peek_binary_operator()
{
  token tok = lex->get_next_token();
  token_t tk = tok.token;
  lex->unget_token(tok);
...
bool csq::parser::peek_literal_with_string()
{
  token_t tk = get_peek_token();
  return (tk == LIT_DECIMAL || tk == LIT_OCTAL
          || tk == LIT_HEX || tk == LIT_BIN
...
bool csq::parser::integer_literal(token_t tk)
{
  return (tk == LIT_DECIMAL || tk == LIT_OCTAL
          || tk == LIT_HEX || tk == LIT_BIN);
}

bool csq::parser::character_literal(token_t tk)
{
  return (tk == LIT_CHAR);
}

bool csq::parser::constant_expression(token_t tk)
{
  return (integer_literal(tk) || character_literal(tk));
}

bool csq::parser::peek_constant_expression()
{
  return constant_expression(get_peek_token());
}

bool csq::parser::peek_assignment_operator()
{
  token_t tk = get_peek_token();
  return assignment_operator(tk);
}

bool csq::parser::peek_identifier()
{
  return (get_peek_token() == IDENTIFIER);
}

bool csq::parser::expect_binary_opertr_token()
{
...
bool csq::parser::expect_literal()
{
...
bool csq::parser::expect_assignment_operator()
{
...
bool csq::parser::member_access_operator(token_t tk)
{
  return (tk == DOT_OP || tk == ARROW_OP);
}

bool csq::parser::peek_member_access_operator()
{
  token_t tk = get_peek_token();
  return member_access_operator(tk);
}

bool csq::parser::expression_token(token_t tk)
{
  return (tk == LIT_DECIMAL || tk == LIT_OCTAL
          || tk == LIT_HEX || tk == LIT_BIN
          || tk == LIT_FLOAT || tk == LIT_CHAR
...
bool csq::parser::peek_expression_token()
{
...
simple-type-specifier :
  ...
  long
*/
bool csq::parser::peek_type_specifier(std::vector<token>& tokens)
{
  token tok;
...
bool csq::parser::type_specifier(token_t tk)
{
...
bool csq::parser::peek_type_specifier()
{
  token_t tk = get_peek_token();
  return type_specifier(tk);
}

void csq::parser::get_type_specifier(std::vector<token>& types)
{
...
bool csq::parser::peek_type_specifier_from(int n)
{
  token *tok = new token[n];
  token_t tk;
  int i;
...
*/
void csq::parser::primary_expression(terminator_t& terminator)
{
  ...
  switch(tok.token){
    case PARENTH_OPEN :
      {
        ...
        if(peek_token(PARENTH_CLOSE)){
          ...
          error::print_error(csq::filename, "expression expected ",
        ...
        if(parenth_stack.size() > 0 && expect_token(PARENTH_CLOSE)){
          if(!check_parenth()){
            error::print_error(csq::filename, "unbalanced parenthesis");
          ...
          if(peek_binary_operator() || peek_unary_operator()){
            ...
            if(!is_expr_terminator_consumed || !is_expr_terminator_got){
              error::print_error(csq::filename,
    ...
    case LIT_CHAR :
      {
        ...
        if(peek_binary_operator() || peek_unary_operator()){
            ...
            if(peek_token(PARENTH_OPEN) || peek_token(IDENTIFIER)){
              ...
              token tok2 = lex->get_next_token();
              error::print_error(csq::filename, "literal or expression expected ",
                                tok2.lexeme, tok2.loc);
              csq::print::print_white_bold_text(expr_list);
              std::cout<<std::endl;
              return;
            }
        ...
        if(peek_token(terminator)){
          ...
          if(peek_token(PARENTH_CLOSE)){
            if(parenth_stack.size()==0){
              token tok2 = lex->get_next_token();
              error::print_error(csq::filename, "error ",
                                  tok2.lexeme, tok2.loc);
            }
    ...
    case BIT_COMPL :
      {
        ...
        if(unary_operator(tok.token)){
          ...
          if(peek_token(PARENTH_OPEN) || peek_expr_literal_token() || peek_token(IDENTIFIER)){
            ...
            token tok2 = lex->get_next_token();
            error::print_error(csq::filename, "literal expected ",
                              tok2.lexeme, tok2.loc);
            return;
          }
        }
    ...
    default :
      {
        error::print_error(csq::filename, "primaryexpr invalid token ",
                            tok.lexeme, tok.loc);
        return;
      }
      break;
...
void csq::parser::sub_primary_expression(terminator_t& terminator)
{
  if(expr_list.size() > 0)
    primary_expression(terminator);
}
...
*/
int csq::parser::operator_precedence(token_t opr)
{
  switch(opr){
    case DOT_OP :
      return 24;
...
void csq::parser::postfix_expression(std::list<token>& postfix_expr)
{
...
struct primary_expr* csq::parser::get_primary_expr_tree()
{
  ...
  if(postfix_expr.size() == 1){
    expr = csq::tree::get_primary_expr_mem();
  ...
  for(post_it = postfix_expr.begin(); post_it != postfix_expr.end(); post_it++){
    ...
    //assign left and right nodes and push the generated tree into stack again
      ...
      oprtr = csq::tree::get_primary_expr_mem();
      oprtr->tok = *post_it;
      oprtr->is_id = false;
      oprtr->is_oprtr = true;
...
struct id_expr* csq::parser::get_id_expr_tree()
{
  ...
  for(post_it = postfix_expr.begin(); post_it != postfix_expr.end(); post_it++){
    if((*post_it).token == IDENTIFIER){
      ...
      expr = nullptr;
            ...
            || (*post_it).token == ARROW_OP){
      oprtr = csq::tree::get_id_expr_mem();
      oprtr->tok = *post_it;
      oprtr->is_id = false;
      oprtr->is_oprtr = true;
      ...
      oprtr->is_oprtr = true;
      oprtr->is_subscript = false;
      oprtr->unary = csq::tree::get_id_expr_mem();
...
*/
void csq::parser::id_expression(terminator_t& terminator)
{
  token tok = lex->get_next_token();
  ...
  if(tok.token == IDENTIFIER){
    ...
    //peek for (
      ...
      error::print_error(csq::filename, st+" expected in id expression but found ",
                          tok.lexeme, tok.loc);
      std::cout<<"    "<<expr_list<<std::endl;
    ...
    error::print_error(csq::filename, " identifier expected but found ",
                      tok.lexeme, tok.loc);
    std::cout<<"    "<<expr_list<<std::endl;
...
*/
void csq::parser::subscript_id_access(terminator_t& terminator)
{
  //expect [
  if(expect_token(SQUARE_OPEN_BRACKET)){
    ...
    if(peek_constant_expression() || peek_identifier()){
      ...
      if(peek_token(SQUARE_OPEN_BRACKET)){
        ...
        error::print_error(csq::filename, "; , ) expected ");
        std::cout<<expr_list<<std::endl;
        return;
      }
    }else{
      token tok2 = lex->get_next_token();
      error::print_error(csq::filename, "constant expression expected ", tok2.lexeme);
      std::cout<<expr_list<<std::endl;
...
pointer-operator :
  *
*/
void csq::parser::pointer_operator_sequence()
{
  token tok;
...
int csq::parser::get_pointer_operator_sequence()
{
  int ptr_count = 0;
  token tok;
  //here ARTHM_MUL token will be changed to PTR_OP
...
*/
void csq::parser::pointer_indirection_access(terminator_t& terminator)
{
  pointer_operator_sequence();
  if(peek_token(IDENTIFIER)){
    id_expression(terminator);
  }else{
    error::print_error(csq::filename, "identifier expected in pointer indirection");
    std::cout<<expr_list<<std::endl;
...
*/
struct id_expr* csq::parser::prefix_incr_expression(terminator_t& terminator)
{
  ...
  if(peek_token(IDENTIFIER)){
    ...
    error::print_error(csq::filename, "identifier expected ");
    std::cout<<expr_list<<std::endl;
...
*/
struct id_expr* csq::parser::prefix_decr_expression(terminator_t& terminator)
{
  struct id_expr* pridexpr = nullptr;
...
*/
void csq::parser::postfix_incr_expression(terminator_t& terminator)
{
  token tok;
  if(expect_token(INCR_OP)){
    token tok = lex->get_next_token();
  ...
  if(peek_token(terminator)){
    ...
    tok = lex->get_next_token();
    error::print_error(csq::filename, "; , ) expected but found "+tok.lexeme, tok.loc);
    std::cout<<expr_list<<std::endl;
...
*/
void csq::parser::postfix_decr_expression(terminator_t& terminator)
{
  if(expect_token(DECR_OP)){
    token tok = lex->get_next_token();
    expr_list.push_back(tok);
...
address-of-expression :
  & id-expression
*/
struct id_expr* csq::parser::address_of_expression(terminator_t& terminator)
{
  ...
  if(expect_token(BIT_AND)){
    ...
    expr_list.push_back(tok);
    id_expression(terminator);
    addrexpr = csq::tree::get_id_expr_mem();
...
sizeof-expression :
  ...
  sizeof ( identifier )
*/
struct sizeof_expr* csq::parser::sizeof_expression(terminator_t& terminator)
{
  struct sizeof_expr* sizeofexpr = csq::tree::get_sizeof_expr_mem();
  ...
  if(peek_type_specifier(simple_types)){
    ...
    error::print_error(csq::filename, "simple types, class names or identifier expected for sizeof ");
    terminator2.clear();
    terminator2.push_back(PARENTH_CLOSE);
...
*/
struct cast_expr* csq::parser::cast_expression(terminator_t& terminator)
{
  struct cast_expr* cstexpr = csq::tree::get_cast_expr_mem();
  token tok ;
  ...
  if(peek_token(IDENTIFIER)){
    ...
    tok = lex->get_next_token();
    error::print_error(csq::filename, " identifier expected in cast expression", tok.loc);
...
void csq::parser::cast_type_specifier(struct cast_expr** expr)
{
  ...
  if(peek_type_specifier(simple_types)){
    ...
    tok = lex->get_next_token();
    error::print_error(csq::filename, "simple type or record name for casting ", tok.loc);
    terminator2.clear();
    terminator2.push_back(PARENTH_CLOSE);
    terminator2.push_back(SEMICOLON);
    terminator2.push_back(COMMA_OP);
...
*/
struct assgn_expr* csq::parser::assignment_expression(terminator_t& terminator,
                                                        bool is_left_side_handled)
{
  ...
  if(expect_assignment_operator()){
    tok = lex->get_next_token();
    assexpr = csq::tree::get_assgn_expr_mem();
    assexpr->tok  = tok;
    ...
    if(!is_left_side_handled){
      ...
      if(ptr_oprtr_count > 0){
        ptr_ind = csq::tree::get_id_expr_mem();
        ptr_ind->is_ptr = true;
...
*/
struct func_call_expr* csq::parser::func_call_expression(terminator_t& terminator)
{
  ...
  funccallexp = csq::tree::get_func_call_expr_mem();
  ...
  if(peek_token(PARENTH_CLOSE)){
    ...
    if(peek_token(terminator)){
      ...
      tok = lex->get_next_token();
      csq::error::print_error(csq::filename,
  ...
  csq::tree::delete_func_call_expr(&funccallexp);
...
*/
void csq::parser::func_call_expression_list(std::list<expr*>& exprlist,
...
expression :
  ...
  function-call-expression
*/
struct expr* csq::parser::expression(terminator_t& terminator)
{
  token tok, tok2;
  std::vector<token> specifier;
  std::list<token>::iterator lst_it;
  terminator_t terminator2;
  struct csq::sizeof_expr* sizeofexpr = nullptr;
  struct csq::cast_expr* castexpr = nullptr;
  struct primary_expr* pexpr = nullptr;
  struct id_expr* idexpr = nullptr;
  struct assgn_expr* assgnexpr = nullptr;
  struct func_call_expr* funcclexpr = nullptr;
  struct expr* _expr = csq::tree::get_expr_mem();
  ...
  switch(tok.token){
    ...
    case BIT_COMPL :
        ...
        if(pexpr == nullptr){
          csq::error::print_error(csq::filename, "error to parse primary expression");
          csq::tree::delete_expr(&_expr);
    ...
    case LIT_STRING :
        pexpr = csq::tree::get_primary_expr_mem();
        pexpr->is_id = false;
        pexpr->tok = tok;
    ...
    case IDENTIFIER :
      ...
      if(peek_token(DOT_OP) || peek_token(ARROW_OP)
        ...
        //peek terminator
          ...
          idexpr = get_id_expr_tree();
          if(idexpr == nullptr){
            csq::error::print_error(csq::filename, "error to parse id expression");
            csq::tree::delete_expr(&_expr);
    ...
    case PARENTH_OPEN :
        ...
        if(type_specifier(tok2.token) || csq::symtable::search_record(csq::record_table, tok2.lexeme)){
          lex->unget_token(tok);
          lex->unget_token(tok2);
          castexpr = cast_expression(terminator);
          if(castexpr == nullptr){
            csq::error::print_error(csq::filename, "error to parse cast expression");
            csq::tree::delete_expr(&_expr);
      ...
      case ARTHM_MUL :
        ...
        if(peek_assignment_operator()){
          assgnexpr = assignment_expression(terminator, false);
          if(assgnexpr == nullptr){
            csq::error::print_error(csq::filename, "error to parse assignment expression");
            csq::tree::delete_expr(&_expr);
      ...
      case INCR_OP :
        ...
        if(idexpr == nullptr){
          csq::error::print_error(csq::filename, "error to parse increment expression");
          csq::tree::delete_expr(&_expr);
        ...
        if(peek_assignment_operator()){
          ...
          if(assgnexpr == nullptr){
            csq::error::print_error(csq::filename, "error to parse passignment expression");
            csq::tree::delete_expr(&_expr);
      ...
      case DECR_OP :
        ...
        if(idexpr == nullptr){
          csq::error::print_error(csq::filename, "error to parse decrement expression");
          csq::tree::delete_expr(&_expr);
      ...
      case BIT_AND :
        ...
        if(idexpr == nullptr){
          csq::error::print_error(csq::filename, "error to parse addressof expression");
          csq::tree::delete_expr(&_expr);
      ...
      case KEY_SIZEOF :
        ...
        if(sizeofexpr == nullptr){
          csq::error::print_error(csq::filename, "error to parse sizeof expression");
          csq::tree::delete_expr(&_expr);
...
*/
void csq::parser::record_specifier()
{
  ...
  if(record_head(&tok, &isglob, &isextrn)){
    ...
    if(csq::symtable::search_record(record_table, tok.lexeme)){
      csq::error::print_error(csq::filename,
    ...
    csq::symtable::insert_record(&record_table, tok.lexeme);
    rec = csq::last_rec_node;
    ...
    csq::error::print_error(csq::filename, "invalid record definition");
...
record-head :
  ...
  record record-name
*/
bool csq::parser::record_head(token* tok, bool* isglob, bool* isextern)
{
...
record-member-definition :
  ...
  void rec-id-list
*/
void csq::parser::record_member_definition(struct st_record_node** rec)
{
  token tok;
  ...
  while((tok = lex->get_next_token()).token != END_OF_FILE){
    ...
    if(peek_type_specifier() || peek_token(IDENTIFIER)){
      ...
      typeinf = csq::symtable::get_type_info_mem();
      ...
      if(types.size() == 1 && types[0].token == IDENTIFIER){
        if(csq::symtable::search_record(csq::record_table, types[0].lexeme)){
          typeinf->type = RECORD_TYPE;
          typeinf->type_specifier.record_type = types[0];
          typeinf->type_specifier.simple_type.clear();
        }else{
          csq::error::print_error(csq::filename,"record '"
...
*/
void csq::parser::rec_id_list(struct st_record_node** rec, struct st_type_info** typeinf)
{
  token tok;
  int ptr_seq = 0;
  ...
  if(peek_token(IDENTIFIER)){
    ...
    tok = lex->get_next_token();
    if(csq::symtable::search_symbol((*rec)->symtab, tok.lexeme)){
      csq::error::print_error(csq::filename, "redeclaration of "+tok.lexeme, tok.loc);
      return;
    }else{
      csq::symtable::insert_symbol(&symt, tok.lexeme);
      assert(csq::last_symbol != nullptr);
      csq::last_symbol->type_info = *typeinf;
      csq::last_symbol->symbol = tok.lexeme;
      csq::last_symbol->tok = tok;
    }
    if(peek_token(SQUARE_OPEN_BRACKET)){
      sublst.clear();
      rec_subscript_member(sublst);
      assert(csq::last_symbol != nullptr);
      csq::last_symbol->is_array = true;
      csq::last_symbol->arr_dimension_list.assign(sublst.begin(), sublst.end());
      sublst.clear();
    }else if(peek_token(COMMA_OP)){
      consume_next_token();
      rec_id_list(&(*rec), &(*typeinf));
  ...
  //peek for *
    ...
    if(peek_token(PARENTH_OPEN)){
      ...
      if(csq::symtable::search_symbol((*rec)->symtab, tok.lexeme)){
        ...
        csq::last_symbol->symbol = tok.lexeme;
        csq::last_symbol->tok = tok;
        csq::last_symbol->is_ptr = true;
        csq::last_symbol->ptr_oprtr_count = ptr_seq;
      }
...
*/
void csq::parser::rec_subscript_member(std::list<token>& sublst)
{
  token tok;
...
*/
void csq::parser::rec_func_pointer_member(struct st_record_node** rec,
                                            int* ptrseq, struct st_type_info** typeinf)
{
  ...
  if(peek_token(IDENTIFIER)){
    ...
    if(csq::symtable::search_symbol((*rec)->symtab, tok.lexeme)){
      ...
      assert(csq::last_symbol != nullptr);
      csq::last_symbol->type_info = *typeinf;
      csq::last_symbol->is_func_ptr = true;
      csq::last_symbol->symbol = tok.lexeme;
      csq::last_symbol->tok = tok;
      csq::last_symbol->ret_ptr_count = *ptrseq;
      ...
      if(peek_token(PARENTH_CLOSE)){
        ...
        rec_func_pointer_params(&(csq::last_symbol));
        expect_token(PARENTH_CLOSE, true);
      }
    }
  }else{
...
*/
void csq::parser::rec_func_pointer_params(struct st_symbol_info** stinf)
{
  token tok;
  std::vector<token> types;
  int ptr_seq = 0;
  ...
  rectype = csq::symtable::get_rec_type_info_mem();
  ...
  if(peek_type_specifier()){
    ...
    csq::symtable::delete_rec_type_info(&rectype);
    tok = lex->get_next_token();
    csq::error::print_error(csq::filename,
...
*/
void csq::parser::simple_declaration(token scope, std::vector<token>& types,
                                        bool is_record_type, struct st_node** st)
{
  token tok;
  struct st_type_info* stype = csq::symtable::get_type_info_mem();
...
*/
void csq::parser::simple_declarator_list(struct st_node** st, struct st_type_info** stinf)
{
  token tok;
  int ptr_seq = 0;
  ...
  if(peek_token(IDENTIFIER)){
    ...
    tok = lex->get_next_token();
    if(csq::symtable::search_symbol((*st), tok.lexeme)){
      csq::error::print_error(csq::filename,
                  "redeclaration/conflicting types of "+tok.lexeme, tok.loc);

      return;
    }else{
      csq::symtable::insert_symbol(&(*st), tok.lexeme);
      if(csq::last_symbol == nullptr) return;
      csq::last_symbol->symbol = tok.lexeme;
      csq::last_symbol->tok = tok;
      csq::last_symbol->type_info = *stinf;
    }
    if(peek_token(SQUARE_OPEN_BRACKET)){
      last_symbol->is_array = true;
      subscript_declarator(&csq::last_symbol);
    }
    if(peek_token(COMMA_OP)){
      consume_next_token();
    ...
    if(peek_token(IDENTIFIER)){
      ...
      if(peek_token(SQUARE_OPEN_BRACKET)){
        ...
        consume_next_token();
        subscript_initializer(csq::last_symbol->arr_init_list);
...
*/
void csq::parser::subscript_declarator(struct st_symbol_info** stsinf)
{
  token tok;
  expect_token(SQUARE_OPEN_BRACKET, true);
  if(peek_constant_expression()){
...
subscript-initializer :
  ...
  { subscript-initializer }
*/
void csq::parser::subscript_initializer(std::vector<std::vector<token>>& arrinit)
{
  token tok;
...
*/
void csq::parser::literal_list(std::vector<token>& ltrl)
{
  token tok;
  if(peek_literal_with_string()){
    tok = lex->get_next_token();
...
function-name :
  identifier
*/
void csq::parser::func_head(struct st_func_info** stfinf, token _funcname, token scope,
                              std::vector<token>& types, bool is_record_type)
{
  token tok;

  *stfinf = csq::symtable::get_func_info_mem();
  ...
  if(!is_record_type){
    (*stfinf)->return_type = csq::symtable::get_type_info_mem();
...
*/
void csq::parser::func_params(std::list<struct st_func_param_info*>& fparams)
{
  token tok;
  std::vector<token> types;
  int ptr_seq = 0;
  ...
  funcparam = csq::symtable::get_func_param_info_mem();
  ...
  if(peek_type_specifier()){
    ...
    csq::symtable::delete_func_param_info(&funcparam);
    tok = lex->get_next_token();
    csq::error::print_error(csq::filename,
...
labled-statement :
  identifier :
*/
struct labled_stmt* csq::parser::labled_statement()
{
  struct labled_stmt* labstmt = csq::tree::get_label_stmt_mem();
  token tok;
  expect_token(IDENTIFIER, false);
  tok = lex->get_next_token();
  labstmt->label = tok;
...
expression-statement :
  expression
*/
struct expr_stmt* csq::parser::expression_statement()
{
  struct expr_stmt* expstmt = csq::tree::get_expr_stmt_mem();
...
*/
struct select_stmt* csq::parser::selection_statement(struct st_node** symtab)
{
  token tok;
  terminator_t terminator = {PARENTH_CLOSE};
  struct select_stmt* selstmt = csq::tree::get_select_stmt_mem();
...
*/
struct iter_stmt* csq::parser::iteration_statement(struct st_node** symtab)
{
  token tok;
  terminator_t terminator = {PARENTH_CLOSE};
  struct iter_stmt* itstmt = csq::tree::get_iter_stmt_mem();
...
jump-statement :
  ...
  goto identifier
*/
struct jump_stmt* csq::parser::jump_statement()
{
  token tok;
  terminator_t terminator = {SEMICOLON};
  struct jump_stmt* jmpstmt = csq::tree::get_jump_stmt_mem();
...
*/
struct asm_stmt* csq::parser::asm_statement()
{
...
*/
void csq::parser::asm_statement_sequence(struct asm_stmt** asmhead)
{
  token tok;
  struct asm_stmt* asmstmt = csq::tree::get_asm_stmt_mem();
  ...
  if(peek_token(SQUARE_OPEN_BRACKET)){
    ...
    if(peek_token(COLON_OP)){
      ...
      tok = lex->get_next_token();
      csq::error::print_error(csq::filename, "output operand expected "+tok.lexeme, tok.loc);
    ...
    if(peek_token(SQUARE_CLOSE_BRACKET)){
      ...
      tok = lex->get_next_token();
      csq::error::print_error(csq::filename, "input operand expected "+tok.lexeme, tok.loc);

      return;
    }

    csq::tree::add_asm_statement(&(*asmhead), &asmstmt);
...
*/
void csq::parser::asm_operand(std::vector<struct asm_operand*>& operand)
{
  token tok;
  terminator_t terminator = {PARENTH_CLOSE};
  struct asm_operand* asmoprd = csq::tree::get_asm_operand_mem();
  expect_token(LIT_STRING, false);
  tok = lex->get_next_token();
...
*/
struct stmt* csq::parser::statement(struct st_node** symtab)
{
  token tok;
  std::vector<token> types;
  ...
  while((tok = lex->get_next_token()).token != END_OF_FILE){
    ...
    //if identifier
      ...
      //if peek for :, call labled statement
        ...
        lex->unget_token(tok);
        statement = csq::tree::get_stmt_mem();
        statement->type = LABEL_STMT;
        statement->labled_statement = labled_statement();
        csq::tree::add_statement(&stmthead, &statement);
        if(peek_token(END_OF_FILE))  return stmthead;
...
void csq::parser::get_func_info(struct st_func_info** func_info, token tok, int type,
          std::vector<token>& types, bool is_extern, bool is_glob)
{
  if(*func_info == nullptr){
    *func_info = csq::symtable::get_func_info_mem();
  }
  (*func_info)->func_name = tok.lexeme;
  (*func_info)->tok = tok;
  (*func_info)->return_type = csq::symtable::get_type_info_mem();
...
*/
struct tree_node* csq::parser::parse()
{
  token tok[5];
  std::vector<token> types;
      ...
      //if global type-specifier
        ...
        if(tok[2].token == IDENTIFIER){
          ...
          if(tok[3].token == PARENTH_OPEN){
            ...
            symtab = csq::symtable::get_node_mem();
            funcinfo = csq::symtable::get_func_info_mem();
            ...
            if(funcit == func_table.end()){
              ...
              expect_token(CURLY_OPEN_BRACKET, true);
              _tree = csq::tree::get_tree_node_mem();
              _tree->symtab = symtab;
              ...
              _tree->symtab = symtab;
              csq::tree::add_tree_node(&tree_head, &_tree);
              expect_token(CURLY_CLOSE_BRACKET, true);
            }else{
              csq::error::print_error(csq::filename,
                            "redeclaration of function "+tok[2].lexeme, tok[2].loc);
              csq::symtable::delete_func_info(&funcinfo);
        ...
        //if * operator
          ...
          if(peek_token(PARENTH_OPEN)){
            csq::symtable::remove_symbol(&global_symtab, funcname.lexeme);
    ...
    //if record type identifier
      ...
      if(tok[1].token == IDENTIFIER){
        ...
        //if * operator
          ...
          if(!csq::symtable::search_record(csq::record_table, tok[0].lexeme)){
            lex->unget_token(tok[1]);
            lex->unget_token(tok[0]);
            _tree = csq::tree::get_tree_node_mem();
            _tree->statement = csq::tree::get_stmt_mem();
            _tree->statement->type = EXPR_STMT;
            _tree->statement->expression_statement = csq::tree::get_expr_stmt_mem();
          ...
          lex->unget_token(tok[0]);
          _tree = csq::tree::get_tree_node_mem();
          csq::symtable::delete_node(&(_tree->symtab));
          _tree->statement = csq::tree::get_stmt_mem();
          _tree->statement->type = EXPR_STMT;
          _tree->statement->expression_statement = csq::tree::get_expr_stmt_mem();
          _tree->statement->expression_statement->expression = expression(terminator);
